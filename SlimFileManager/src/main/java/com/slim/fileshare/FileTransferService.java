...
public class FileTransferService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent.getAction().equals(ACTION_SEND_FILE)) {
            ...
            try {
                Log.d("Opening client socket - ");
                socket.bind(null);
                socket.connect((new InetSocketAddress(host, port)), SOCKET_TIMEOUT);

                Log.d("Client socket - " + socket.isConnected());
                ...
                Log.d("Client: Data written");
                oos.close();	//close the ObjectOutputStream after sending data.
            } catch (IOException e) {
                Log.e(e.getMessage());
                e.printStackTrace();
                CommonMethods.e("Unable to connect host",
                        "service socket error in wififiletransferservice class");
           	 mHandler.post(new Runnable() {
					
					public void run() {
						Toast.makeText(FileTransferService.this,
                                "Paired Device is not Ready to receive the file",
